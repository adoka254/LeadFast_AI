    ...
    try {
      ...
      if (isSignUp) {
        ...
        const res = await fetch('/api/register', {
          ...
          body: JSON.stringify({ userId, business_name: businessName, trade: businessTrade, contact_email: businessContactEmail || email, contact_phone: businessContactPhone, plan: businessPlan || null }),
        });
      ...
      if (data.session) {
        ...
        const bizRes = await fetch('/api/my-businesses', { headers: { Authorization: `Bearer ${data.session.access_token}` } });
        const bizData = await bizRes.json();
        const bizList = Array.isArray(bizData) ? bizData : [];
        setBusinesses(bizList);

        if (bizList.length === 0) { router.replace('/'); return; }
        if (bizList.length === 1) {
    ...
    <main>
      <section className="container" style={{ padding: '40px 0', display: 'flex', justifyContent: 'center' }}>
        <div className="panel card" style={{ width: '100%', maxWidth: '460px' }}>
          ...
          <button type="button" onClick={() => { setIsSignUp(!isSignUp); setShowBizSelect(false); }}
          ...
          <p style={{ color: '#94a3b8', marginBottom: '18px' }}>Only registered contractors can access the private dashboard.</p>
          ...
          {showBizSelect && businesses.length > 0 ? (
            <div style={{ display: 'grid', gap: '12px' }}>
              ...
              <button type="button" className="btn" style={{ background: 'linear-gradient(135deg, #2563eb, #38bdf8)', color: 'white', padding: '10px 14px' }}
                onClick={() => {
              ...
              {isSignUp && (
                <>
                  <label style={{ display: 'grid', gap: '6px' }}>
                    ...
                    <input required type="text" value={businessName} onChange={(e) => setBusinessName(e.target.value)} placeholder="Acme Corp" />
                  </label>
                  <label style={{ display: 'grid', gap: '6px' }}>
                    <span>Trade</span>
                  ...
                  <label style={{ display: 'grid', gap: '6px' }}>
                    ...
                    <input type="email" value={businessContactEmail} onChange={(e) => setBusinessContactEmail(e.target.value)} placeholder="info@acmecorp.com" />
                  </label>
                  <label style={{ display: 'grid', gap: '6px' }}>
