  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const fullMessage = selectedContractor
        ? `[Request for ${selectedContractor.business_name}] ${formData.message}`
        : formData.message;
      ...
      const response = await fetch('/api/lead-intake', {
        ...
        body: JSON.stringify({
          ...
          message: fullMessage,
          businessId: selectedContractor?.id ?? null
        })
      });
        ...
        <div className="grid grid-2" style={{ alignItems: 'start' }}>
              ...
              {contractorsLoading ? (
                ...
                visibleContractors.map((c) => (
                  <div
                    key={c.id}
                    style={{
                      border: selectedContractor?.id === c.id ? '2px solid var(--accent-2)' : '1px solid var(--border)',
                      borderRadius: '14px',
                      padding: '16px',
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
                      ...
                      <button
                        ...
                        style={{
                          background: selectedContractor?.id === c.id ? 'var(--accent-2)' : 'rgba(255,255,255,0.06)',
                          color: selectedContractor?.id === c.id ? '#07111f' : '#f8fafc',
                          fontSize: '0.85rem',
                          fontWeight: '600',
                          padding: '6px 14px'
                        }}
                      >
                        {selectedContractor?.id === c.id ? '✓ Selected' : 'Request Quote'}
                      </button>
                    </div>
                  </div>
                ))
          ...
          <div className="panel card">
            ...
            <p style={{ color: '#94a3b8', marginBottom: '16px', fontSize: '0.9rem' }}>
              {selectedContractor 
                ? `Direct request to ${selectedContractor.business_name}` 
              ...
              {selectedContractor && (
                <div style={{ padding: '8px 12px', background: 'rgba(56, 189, 248, 0.1)', border: '1px solid var(--accent-2)', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.85rem', color: '#38bdf8' }}>
                    Target Contractor: <strong>{selectedContractor.business_name}</strong>
                  </span>
                  <button type="button" onClick={() => setSelectedContractor(null)} style={{ background: 'none', border: 'none', color: '#94a3b8', cursor: 'pointer', fontSize: '0.8rem' }}>
                    Clear
                  </button>
                </div>
              )}
